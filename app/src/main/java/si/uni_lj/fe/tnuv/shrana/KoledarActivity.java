...
public class KoledarActivity extends AppCompatActivity {
    ...
    private final SimpleDateFormat zapisMeseca = new SimpleDateFormat("MMMM yyyy", new Locale("sl", "SI"));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getOnBackPressedDispatcher().addCallback(this, new OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                Intent intent = new Intent(KoledarActivity.this, MainActivity.class);
    ...
    private void prikaziKoledar() {
        ...
        for (int i = 0; i < zamik; i++) koledarMreza.addView(praznaCelica());
        int leto = prikazanMesec.get(Calendar.YEAR);
        int mesec = prikazanMesec.get(Calendar.MONTH);
        for (int dan = 1; dan <= steviloDni; dan++) koledarMreza.addView(celicaDneva(leto, mesec, dan));
    }
    ...
    private View celicaDneva(int leto, int mesec, int dan) {
        ...
        if (izbran) stevilka.setTypeface(null, Typeface.BOLD);
        ...
        for (int i = 0; i < steviloObrokov; i++) pikice.addView(pikica());
    ...
    private GradientDrawable ozadjeDneva(boolean izbran, boolean danes) {
        ...
        if (izbran) {
            ...
            if (danes) ozadje.setStroke(dp(1), getColorCompat(R.color.locnica));
        }
        return ozadje;
    }

    private boolean jeIzbraniDan(int leto, int mesec, int dan) {
        return izbranDan.get(Calendar.YEAR) == leto && izbranDan.get(Calendar.MONTH) == mesec && izbranDan.get(Calendar.DAY_OF_MONTH) == dan;
    }

    private boolean jeDanes(int leto, int mesec, int dan) {
        Calendar danes = Calendar.getInstance();
        return danes.get(Calendar.YEAR) == leto && danes.get(Calendar.MONTH) == mesec && danes.get(Calendar.DAY_OF_MONTH) == dan;
    }

    private int steviloObrokovZaDatum(String datum) {
        int stevilo = 0;
        for (String obrok : OBROKI) {
            if (nacrt.containsKey(datum + "|" + obrok)) stevilo++;
        }
        return stevilo;
    }
    ...
    private int dp(int vrednost) { return (int) (vrednost * getResources().getDisplayMetrics().density + 0.5f); }
    private int getColorCompat(int id) { return getResources().getColor(id, getTheme()); }

    private void prikaziObroke() {
        vsebinaObrokov.removeAllViews();
        for (String obrok : OBROKI) {
            View vrstica = LayoutInflater.from(this).inflate(R.layout.item_obrok, vsebinaObrokov, false);
    ...
    private void izberiRecept(String obrok) {
        ...
        if (recepti.isEmpty()) {
            Toast.makeText(this, "Najprej dodaj recept v zavihku Recepti", Toast.LENGTH_SHORT).show();
            return;
        }
        String[] imena = new String[recepti.size()];
        for (int i = 0; i < recepti.size(); i++) imena[i] = recepti.get(i).naslov;
    ...
    private void nastaviNavigacijo() {
        ...
        navigacija.setOnItemSelectedListener(item -> {
            ...
            if (id == R.id.nav_koledar) return true;
            if (id == R.id.nav_recepti) { startActivity(new Intent(this, MainActivity.class)); finish(); return true; }
            if (id == R.id.nav_seznam) { startActivity(new Intent(this, SeznamActivity.class)); finish(); return true; }
            if (id == R.id.nav_casovnik) { startActivity(new Intent(this, CasovnikiActivity.class)); finish(); return true; }
            if (id == R.id.nav_pomocnik) { startActivity(new Intent(this, PomocnikActivity.class)); finish(); return true; }
            return true;
        });
    }
    ...
    private void naloziNacrt() {
        ...
        Map<String, String> shranjen = JsonShramba.nalozi(this, IME_DATOTEKE, KLJUC, tip);
        if (shranjen != null) nacrt = shranjen;
    }

    private void shraniNacrt() { JsonShramba.shrani(this, IME_DATOTEKE, KLJUC, nacrt); }
}